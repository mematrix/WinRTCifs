...
using System.Net;
using System.Text;
using WinrtCifs.Dcerpc;
using WinrtCifs.Dcerpc.Msrpc;
using WinrtCifs.Netbios;
...
namespace WinrtCifs.Smb
{
    ...
    public class SmbFile : UrlConnection
    {
        ...
        /// <exception cref="WinrtCifs.Smb.SmbException"></exception>
        public virtual List<string> List(ISmbFilenameFilter filter)
        {
        ...
        /// </returns>
        /// <exception cref="SmbException"></exception>
        public virtual List<SmbFile> ListFiles()
        {
        ...
        /// </returns>
        /// <exception cref="SmbException"></exception>
        public virtual List<SmbFile> ListFiles(string wildcard)
        {
        ...
        /// <exception cref="SmbException">SmbException</exception>
        /// <exception cref="WinrtCifs.Smb.SmbException"></exception>
        public virtual List<SmbFile> ListFiles(ISmbFilenameFilter filter)
        {
        ...
        /// <returns>An array of <tt>SmbFile</tt> objects</returns>
        /// <exception cref="WinrtCifs.Smb.SmbException"></exception>
        public virtual List<SmbFile> ListFiles(ISmbFileFilter filter)
        {
            return ListFiles("*", AttrDirectory | AttrHidden | AttrSystem, null, filter);
        }

        /// <exception cref="WinrtCifs.Smb.SmbException"></exception>
        internal virtual List<string> List(string wildcard, int searchAttributes, ISmbFilenameFilter
             fnf, ISmbFileFilter ff)
        {
            List<object> list = new List<object>();
            DoEnum(list, false, wildcard, searchAttributes, fnf, ff);
            return list.OfType<string>().ToList(); ; //Collections.ToArray<string>(list);
        }

        /// <exception cref="WinrtCifs.Smb.SmbException"></exception>
        internal virtual List<SmbFile> ListFiles(string wildcard, int searchAttributes
            , ISmbFilenameFilter fnf, ISmbFileFilter ff)
        {
            List<object> list = new List<object>();
            DoEnum(list, true, wildcard, searchAttributes, fnf, ff);

            return list.OfType<SmbFile>().ToList(); ; //Collections.ToArray<SmbFile>(list);
        }
             ...
             searchAttributes, ISmbFilenameFilter fnf, ISmbFileFilter ff)
        {
            ...
            foreach (var item in map.Keys)
            {
                ...
                if (name.Length > 0)
                {
                    ...
                    SmbFile f = new SmbFile(this, name, e.GetFilesystemType(), AttrReadonly
        ...
        internal virtual void Delete(string fileName)
        {
            ...
            if ((_attributes & AttrDirectory) != 0)
            {
                try
                {
                    List<SmbFile> l = ListFiles("*", AttrDirectory | AttrHidden | AttrSystem, null, null
                        );
