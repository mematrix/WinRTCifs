...
namespace WinrtCifs.Util.Sharpen
{
    public class SocketEx : Socket
    {
        ...
        public void ConnectOld(IPEndPoint endPoint, int timeOut)
        {
        ...
        public int ReceiveOld(byte[] buffer, int offset, int count)
        {
            using (var evt = new ManualResetEventSlim(false))
            using (var args = new SocketAsyncEventArgs
            {
        ...
        public void SendOld(byte[] buffer, int offset, int length, EndPoint destination = null)
        {
            using (var evt = new ManualResetEventSlim(false))
