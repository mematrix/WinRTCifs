...
namespace WinrtCifs.Netbios
{
	public class Lmhosts
	{
		...
		/// <see cref="WinrtCifs.UniAddress">Jcifs.UniAddress</see>
		/// . It does
		/// not throw an
			...
			lock (typeof(Lmhosts))
			{
				...
				try
				{
					if (Filename != null)
					{
                        ...
                        long lm= fileInfo.LastAccessTimeUtc.ToMillisecondsSinceEpoch();
						...
						if (lm > _lastModified)
						{
							...
							Tab.Clear();
							_alt = 0;
							Populate(new StreamReader(file));
						}
		...
		internal static void Populate(StreamReader br)
		{
			string line;
